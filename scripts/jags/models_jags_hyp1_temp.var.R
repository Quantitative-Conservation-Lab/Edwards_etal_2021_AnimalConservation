...
# These models address the hypothesis that the environmental predictor temp.var measured in the nest 
# affect the survival of the eggs.
# Models are autoregressive 
# JAGS scripts are saved as txt files within the folder scripts/jags/jags_models/temp.var
...
# Output is "sink" to file "model_outputs/jags/compare_temp.var.txt" 
# Plots are saved in model_outputs/jags/temp.var
#-----------------------------------------------------
...
if(!dir.exists("scripts/jags/jags_models/temp.var/")){
    dir.create("scripts/jags/jags_models/temp.var/")
}
...
parameters_quadratic <- c("log.like.id", "beta.int","beta.fac","beta.temp.var","beta.temp.var2", "mu.mn.temp.var","rho.mn.temp.var","sigma.temp.var",
                         "alpha.Treat", "sigma.treat")
parameters_linear <- c("log.like.id","beta.int","beta.fac","beta.temp.var","mu.mn.temp.var","rho.mn.temp.var","sigma.temp.var",
                        "alpha.Treat", "sigma.treat")
parameters_null <- c("log.like.id", "beta.int","beta.fac", "mu.mn.temp.var","rho.mn.temp.var","sigma.temp.var",
                      "alpha.Treat", "sigma.treat")
...
for(i in 1:nind){  
  ...
  for(t in (first[i]+1):last[i]){
    ...
    + beta.fac[facility[i]] + beta.temp.var*temp.var[i,t-1] + beta.temp.var2*temp.var[i,t-1]*temp.var[i,t-1]

    temp.var[i,t] ~ dnorm(mn.temp.var[i,t],tau.temp.var[egg.treatment[i]])

    mn.temp.var[i,t] <- mu.mn.temp.var + rho.mn.temp.var*(temp.var[i,t-1]-mu.mn.temp.var)
...
beta.temp.var ~ dnorm(0,0.01)
beta.temp.var2 ~ dnorm(0,0.01)
for(f in 1:8){
  tau.temp.var[f] <- pow(sigma.temp.var[f],-2)
  sigma.temp.var[f] ~ dunif(0,5)
}

mu.mn.temp.var ~ dnorm(0,0.01)
rho.mn.temp.var ~ dnorm(0,0.01)
...
",file="scripts/jags/jags_models/temp.var/nestmodel_quadratic.txt")
...
for(i in 1:nind){  
  ...
  for(t in (first[i]+1):last[i]){
    ...
    + beta.fac[facility[i]] + beta.temp.var*temp.var[i,t-1] 
...
",file="scripts/jags/jags_models/temp.var/nestmodel_linear.txt")
...
jagsfit.1.quadratic <- jags(data=dataset, inits=inits, parameters.to.save=parameters_quadratic, 
                  ...
                  model.file="scripts/jags/jags_models/temp.var/nestmodel_quadratic.txt", parallel=TRUE)

jagsfit.1.linear <- jags(data=dataset, inits=inits, parameters.to.save=parameters_linear, 
                          n.chains=nc, n.burnin = nb, n.iter=ni, n.thin=nt, n.adapt = na,
                          model.file="scripts/jags/jags_models/temp.var/nestmodel_linear.txt", parallel=TRUE)
...
#######################################################################################
#################### MODEL COMPARISON
if(file.exists("model_outputs/jags/compare_temp.var.txt")) unlink("model_outputs/jags/compare_temp.var.txt")
sink(file = "model_outputs/jags/compare_temp.var.txt")
print("--------------------------------------------------------------------")
...
####################################################################
print("compare null model with linear and quadratic for variable temp.var ")
...
print("Compare null with linear for variable temp.var")
...
print("Summary quadratic model temp.var")
...
print("Summary linear model temp.var")
...
pdf(file="model_outputs/jags/plots/jagsfit.temp.var_quadratic.pdf")
plot(jagsfit.1.quadratic$samples[,para_quadratic])
dev.off()
pdf(file="model_outputs/jags/plots/jagsfit.temp.var_linear.pdf")
...
print("#############################################################mo")
save(mean_hdi_quadratic, mean_hdi_linear, mean_hdi_null, file = "model_outputs/jags/mean_hdi/means_hdi_temp.var.RData")
print("###################################################################")
print("###################################################################")
# Create linear plots
load(file = "model_outputs/jags/mean_hdi/means_hdi_temp.var.RData")
load(file = "data/derived_data/dataset_hyp_1_40min.RData")
nvalues = 2000
expit <- function (x) exp(x)/(1 + exp(x))
b_length_new <- seq(min(dataset$temp.var,na.rm=T), max(dataset$temp.var,na.rm=T), 
                    length.out=nvalues)
lower = expit(mean_hdi_linear["beta.temp.var", 2] * b_length_new)
upper =expit(mean_hdi_linear["beta.temp.var", 3] * b_length_new)
means = expit(mean_hdi_linear["beta.temp.var", 1] * b_length_new)

means_quadratic = expit(mean_hdi_quadratic["beta.temp.var", 1] * b_length_new +
                          mean_hdi_quadratic["beta.temp.var2", 1] * b_length_new * b_length_new)

# Plot linear and quadratic functions
pdf(file="model_outputs/jags/plots/regression_temp.var_linear_versus-quadratic.pdf")
plot(x=dataset$temp.var, y=jitter(dataset$enc.hist, 0.2), ylab ="Egg survival", 
     xlab = "Temperature Variance", pch=20)

lines(y = means, x=b_length_new, col=viridis(2)[1])
lines(y=means_quadratic, x = b_length_new, col=viridis(2)[2])
legend(x=min(dataset$temp.var, na.rm=T)-0.15*min(dataset$temp.var, na.rm=T), y=0.3,  
       col=viridis(2), legend=c("linear", "quadratic"), lty=c(1,1))
dev.off()

# Plot linear functions, including hdi
pdf(file="model_outputs/jags/plots/regression_temp.var_linear_2.pdf")
plot(x=dataset$temp.var, y=jitter(dataset$enc.hist, 0.2), ylab ="Egg survival", 
     xlab = "Temperature Variance", pch=20)
...
ymax = expit(max(fac_int[,1], treat_int[,1]) -mean_hdi_linear["beta.int",1] + mean_hdi_linear["beta.temp.var", 1] * b_length_new)
ymin = expit(min(fac_int[,1], treat_int[,1]) -mean_hdi_linear["beta.int",1] + mean_hdi_linear["beta.temp.var", 1] * b_length_new)
# plot linear function including variation between facilities and treatments
pdf(file="model_outputs/jags/plots/regression_temp.var_linear_1.pdf")
plot(x=dataset$temp.var, y=jitter(dataset$enc.hist, 0.2), ylab ="Egg survival", 
     xlab = "Temperature Variance", pch=20)
polygon(c(b_length_new,rev(b_length_new)),c(ymin,rev(ymax)),
        col=viridis(1, alpha=0.4), border=NA)
lines(y = means, x=b_length_new, col=viridis(1)[1])
legend(x=min(dataset$temp.var, na.rm=T)-0.15*min(dataset$temp.var, na.rm=T),
       y=0.5,  
