...
# These models address the hypothesis that the environmental predictor temp.mn measured in the nest 
# affect the survival of the eggs.
# Models are autoregressive 
# JAGS scripts are saved as txt files within the folder scripts/jags/jags_models/temp.mn
...
# Output is "sink" to file "model_outputs/jags/compare_temp.mn.txt" 
# Plots are saved in model_outputs/jags/temp.mn
#-----------------------------------------------------
...
if(!dir.exists("scripts/jags/jags_models/temp.mn/")){
    dir.create("scripts/jags/jags_models/temp.mn/")
}
...
for(i in 1:nind){  
  ...
  for(t in (first[i]+1):last[i]){
    ...
    + beta.fac[facility[i]] + beta.temp.mn*temp.mn[i,t-1] + beta.temp.mn2*temp.mn[i,t-1]*temp.mn[i,t-1]

    temp.mn[i,t] ~ dnorm(mn.temp.mn[i,t],tau.temp.mn[egg.treatment[i]])

    mn.temp.mn[i,t] <- mu.mn.temp.mn + rho.mn.temp.mn*(temp.mn[i,t-1]-mu.mn.temp.mn)
...
beta.temp.mn ~ dnorm(0,0.01)
beta.temp.mn2 ~ dnorm(0,0.01)
for(f in 1:8){
  tau.temp.mn[f] <- pow(sigma.temp.mn[f],-2)
  sigma.temp.mn[f] ~ dunif(0,5)
}

mu.mn.temp.mn ~ dnorm(0,0.01)
rho.mn.temp.mn ~ dnorm(0,0.01)
...
",file="scripts/jags/jags_models/temp.mn/nestmodel_quadratic.txt")
...
",file="scripts/jags/jags_models/temp.mn/nestmodel_linear.txt")
...
jagsfit.1.quadratic <- jags(data=dataset, inits=inits, parameters.to.save=parameters_quadratic, 
                  ...
                  model.file="scripts/jags/jags_models/temp.mn/nestmodel_quadratic.txt", parallel=TRUE)

jagsfit.1.linear <- jags(data=dataset, inits=inits, parameters.to.save=parameters_linear, 
                          n.chains=nc, n.burnin = nb, n.iter=ni, n.thin=nt, n.adapt = na,
                          model.file="scripts/jags/jags_models/temp.mn/nestmodel_linear.txt", parallel=TRUE)
...
####################################################################
print("compare null model with linear and quadratic for variable temp.mn ")
...
print("Compare null with linear for variable temp.mn")
...
print("Summary quadratic model temp.mn")
...
print("Summary linear model temp.mn")
...
pdf(file="model_outputs/jags/plots/jagsfit.temp.mn_quadratic.pdf")
plot(jagsfit.1.quadratic$samples[,para_quadratic])
dev.off()
pdf(file="model_outputs/jags/plots/jagsfit.temp.mn_linear.pdf")
...
print("#############################################################mo")
save(mean_hdi_quadratic, mean_hdi_linear, mean_hdi_null, file = "model_outputs/jags/mean_hdi/means_hdi_temp.mn.RData")
print("###################################################################")
print("###################################################################")
# Create linear plots
load(file = "model_outputs/jags/mean_hdi/means_hdi_temp.mn.RData")
load(file = "data/derived_data/dataset_hyp_1_40min.RData")
nvalues = 2000
expit <- function (x) exp(x)/(1 + exp(x))
b_length_new <- seq(min(dataset$temp.mn,na.rm=T), max(dataset$temp.mn,na.rm=T), 
                    length.out=nvalues)
lower = expit(mean_hdi_linear["beta.temp.mn", 2] * b_length_new)
upper =expit(mean_hdi_linear["beta.temp.mn", 3] * b_length_new)
means = expit(mean_hdi_linear["beta.temp.mn", 1] * b_length_new)

means_quadratic = expit(mean_hdi_quadratic["beta.temp.mn", 1] * b_length_new +
                          mean_hdi_quadratic["beta.temp.mn2", 1] * b_length_new * b_length_new)

# Plot linear and quadratic functions
pdf(file="model_outputs/jags/plots/regression_temp.mn_linear_versus-quadratic.pdf")
plot(x=dataset$temp.mn, y=jitter(dataset$enc.hist, 0.2), ylab ="Egg survival", 
     xlab = "Temperature Mean", pch=20)

lines(y = means, x=b_length_new, col=viridis(2)[1])
lines(y=means_quadratic, x = b_length_new, col=viridis(2)[2])
legend(x=min(dataset$temp.mn, na.rm=T)-0.15*min(dataset$temp.mn, na.rm=T), y=0.3,  
       col=viridis(2), legend=c("linear", "quadratic"), lty=c(1,1))
dev.off()

# Plot linear functions, including hdi
pdf(file="model_outputs/jags/plots/regression_temp.mn_linear_2.pdf")
plot(x=dataset$temp.mn, y=jitter(dataset$enc.hist, 0.2), ylab ="Egg survival", 
     xlab = "Temperature Mean", pch=20)
...
ymax = expit(max(fac_int[,1], treat_int[,1]) -mean_hdi_linear["beta.int",1] + mean_hdi_linear["beta.temp.mn", 1] * b_length_new)
ymin = expit(min(fac_int[,1], treat_int[,1]) -mean_hdi_linear["beta.int",1] + mean_hdi_linear["beta.temp.mn", 1] * b_length_new)
# plot linear function including variation between facilities and treatments
#Figure one in ms
vart <- read.csv("data/Egg_summary_survival_dailydata_40min.csv")
vart <- vart$temp_mean
pdf(file="model_outputs/jags/plots/regression_temp.mn_linear_1.pdf")
plot(x=dataset$temp.mn, y=jitter(dataset$enc.hist, 0.2), ylab ="Probability of a hatch", 
     xlab = "Daily mean temperature (°C)", pch=20, xaxt="n")
xtick<- round(seq(round(min(dataset$temp.mn, na.rm = T), 2), round(max(dataset$temp.mn, na.rm = T), 2), 
           length.out = 8), 2)
xtick_untransformed <- round((xtick * sd(vart, na.rm = T)) + mean(vart, na.rm=T), 2)
axis(side=1, at=xtick[c(1,3,5,7)], labels = xtick_untransformed[c(1,3,5,7)])
axis(side=1, at=xtick, labels = F)
polygon(c(b_length_new,rev(b_length_new)),c(ymin,rev(ymax)),
        col=viridis(1, alpha=0.4), border=NA)
lines(y = means, x=b_length_new, col=viridis(1)[1])
legend(x=min(dataset$temp.mn, na.rm=T)-0.15*min(dataset$temp.mn, na.rm=T),
       y=0.5,  
       col=c(viridis(1), viridis(1, alpha=0.4)), legend=c("Mean", 
      "Variation between incubation\n treatments and egg type"), 
