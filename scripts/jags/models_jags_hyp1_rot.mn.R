...
# These models address the hypothesis that the environmental predictor rot.mn measured in the nest 
# affect the survival of the eggs.
# Models are autoregressive 
# JAGS scripts are saved as txt files within the folder scripts/jags/jags_models/rot.mn
...
# Output is "sink" to file "model_outputs/jags/compare_rot.mn.txt" 
# Plots are saved in model_outputs/jags/rot.mn
#-----------------------------------------------------
...
if(!dir.exists("scripts/jags/jags_models/rot.mn/")){
    dir.create("scripts/jags/jags_models/rot.mn/")
}
...
parameters_quadratic <- c("log.like.id", "beta.int","beta.fac","beta.rot.mn","beta.rot.mn2", "mu.mn.rot.mn","rho.mn.rot.mn","sigma.rot.mn",
                         "alpha.Treat", "sigma.treat")
parameters_linear <- c("log.like.id","beta.int","beta.fac","beta.rot.mn","mu.mn.rot.mn","rho.mn.rot.mn","sigma.rot.mn",
                        "alpha.Treat", "sigma.treat")
parameters_null <- c("log.like.id", "beta.int","beta.fac", "mu.mn.rot.mn","rho.mn.rot.mn","sigma.rot.mn",
                      "alpha.Treat", "sigma.treat")
...
for(i in 1:nind){  
  ...
  for(t in (first[i]+1):last[i]){
    ...
    + beta.fac[facility[i]] + beta.rot.mn*rot.mn[i,t-1] + beta.rot.mn2*rot.mn[i,t-1]*rot.mn[i,t-1]

    rot.mn[i,t] ~ dnorm(mn.rot.mn[i,t],tau.rot.mn[egg.treatment[i]])

    mn.rot.mn[i,t] <- mu.mn.rot.mn + rho.mn.rot.mn*(rot.mn[i,t-1]-mu.mn.rot.mn)
...
beta.rot.mn ~ dnorm(0,0.01)
beta.rot.mn2 ~ dnorm(0,0.01)
for(f in 1:8){
  tau.rot.mn[f] <- pow(sigma.rot.mn[f],-2)
  sigma.rot.mn[f] ~ dunif(0,5)
}

mu.mn.rot.mn ~ dnorm(0,0.01)
rho.mn.rot.mn ~ dnorm(0,0.01)
...
",file="scripts/jags/jags_models/rot.mn/nestmodel_quadratic.txt")
...
for(i in 1:nind){  
  ...
  for(t in (first[i]+1):last[i]){
    ...
    + beta.fac[facility[i]] + beta.rot.mn*rot.mn[i,t-1] 
...
",file="scripts/jags/jags_models/rot.mn/nestmodel_linear.txt")
...
jagsfit.1.quadratic <- jags(data=dataset, inits=inits, parameters.to.save=parameters_quadratic, 
                  ...
                  model.file="scripts/jags/jags_models/rot.mn/nestmodel_quadratic.txt", parallel=TRUE)

jagsfit.1.linear <- jags(data=dataset, inits=inits, parameters.to.save=parameters_linear, 
                          n.chains=nc, n.burnin = nb, n.iter=ni, n.thin=nt, n.adapt = na,
                          model.file="scripts/jags/jags_models/rot.mn/nestmodel_linear.txt", parallel=TRUE)
...
#######################################################################################
#################### MODEL COMPARISON
if(file.exists("model_outputs/jags/compare_rot.mn.txt")) unlink("model_outputs/jags/compare_rot.mn.txt")
sink(file = "model_outputs/jags/compare_rot.mn.txt")
print("--------------------------------------------------------------------")
...
####################################################################
print("compare null model with linear and quadratic for variable rot.mn ")
...
print("Compare null with linear for variable rot.mn")
...
print("Summary quadratic model rot.mn")
...
print("Summary linear model rot.mn")
...
pdf(file="model_outputs/jags/plots/jagsfit.rot.mn_quadratic.pdf")
plot(jagsfit.1.quadratic$samples[,para_quadratic])
dev.off()
pdf(file="model_outputs/jags/plots/jagsfit.rot.mn_linear.pdf")
...
print("#############################################################mo")
save(mean_hdi_quadratic, mean_hdi_linear, mean_hdi_null, file = "model_outputs/jags/mean_hdi/means_hdi_rot.mn.RData")
print("###################################################################")
print("###################################################################")
# Create linear plots
load(file = "model_outputs/jags/mean_hdi/means_hdi_rot.mn.RData")
load(file = "data/derived_data/dataset_hyp_1_40min.RData")
nvalues = 2000
expit <- function (x) exp(x)/(1 + exp(x))
b_length_new <- seq(min(dataset$rot.mn,na.rm=T), max(dataset$rot.mn,na.rm=T), 
                    length.out=nvalues)
lower = expit(mean_hdi_linear["beta.rot.mn", 2] * b_length_new)
upper =expit(mean_hdi_linear["beta.rot.mn", 3] * b_length_new)
means = expit(mean_hdi_linear["beta.rot.mn", 1] * b_length_new)

means_quadratic = expit(mean_hdi_quadratic["beta.rot.mn", 1] * b_length_new +
                          mean_hdi_quadratic["beta.rot.mn2", 1] * b_length_new * b_length_new)

# Plot linear and quadratic functions
pdf(file="model_outputs/jags/plots/regression_rot.mn_linear_versus-quadratic.pdf")
plot(x=dataset$rot.mn, y=jitter(dataset$enc.hist, 0.2), ylab ="Egg survival", 
     xlab = "Rotation Mean", pch=20)

lines(y = means, x=b_length_new, col=viridis(2)[1])
lines(y=means_quadratic, x = b_length_new, col=viridis(2)[2])
legend(x=min(dataset$rot.mn, na.rm=T)-0.15*min(dataset$rot.mn, na.rm=T), y=0.3,  
       col=viridis(2), legend=c("linear", "quadratic"), lty=c(1,1))
dev.off()

# Plot linear functions, including hdi
pdf(file="model_outputs/jags/plots/regression_rot.mn_linear_2.pdf")
plot(x=dataset$rot.mn, y=jitter(dataset$enc.hist, 0.2), ylab ="Egg survival", 
     xlab = "Rotation Mean", pch=20)
...
ymax = expit(max(fac_int[,1], treat_int[,1]) -mean_hdi_linear["beta.int",1] + mean_hdi_linear["beta.rot.mn", 1] * b_length_new)
ymin = expit(min(fac_int[,1], treat_int[,1]) -mean_hdi_linear["beta.int",1] + mean_hdi_linear["beta.rot.mn", 1] * b_length_new)
# plot linear function including variation between facilities and treatments
pdf(file="model_outputs/jags/plots/regression_rot.mn_linear_1.pdf")
plot(x=dataset$rot.mn, y=jitter(dataset$enc.hist, 0.2), ylab ="Egg survival", 
     xlab = "Rotation Mean", pch=20)
polygon(c(b_length_new,rev(b_length_new)),c(ymin,rev(ymax)),
        col=viridis(1, alpha=0.4), border=NA)
lines(y = means, x=b_length_new, col=viridis(1)[1])
legend(x=min(dataset$rot.mn, na.rm=T)-0.15*min(dataset$rot.mn, na.rm=T),
       y=0.5,  
