---
title: "Combining data-logger egg data"
...
#Direct the computer to the folder where the data is stored
...
```
##For each egg create a new dataset with the 40th datapoint every 24 hours
#starting from the first data point recorded each day and put it in egg_data1 
...
for(i in 1:length(eggs_all)){
  ...
  #S17#1, S17#2,S2#2, Y42#4, Y42#5-have a small time gap (couple hours) so do not have 
  #data every 40 mins during that time
...
```
#Calculate the 24hr mean and variance for temperature, egg turning and humidity
```{r, echo=TRUE, results='hide'}
egg_data2 = list()
for(j in 1:length(egg_data1)){
  ...
  egg_data_summs<-cbind(egg_data_summs, Daily_temp_var$temp_var, Daily_rh_mean$rh_mean, 
                        Daily_rh_var$rh_var)
  #Rename
  egg_data_summs<-rename(egg_data_summs, c("Daily_temp_var$temp_var"="temp_var",  "Daily_rh_mean$rh_mean"="rh_mean", "Daily_rh_var$rh_var"="rh_var"))
  
  egg_data2[[j]] <- egg_data_summs
  
}
```
#Add paired and unpaired date and write Egg_summary_survival_dailydata_40min.csv
```{r, echo=TRUE, results='hide'}
...
```
#Add paired date (not in the individual egg csvs) and write Egg_summary_survival_predictors.csv
```{r, echo=TRUE, results='hide'}
#Add paired date from another summary datasheet
