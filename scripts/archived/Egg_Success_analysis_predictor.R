...
#########################################################################################
#read the data out and read it back in as this will be the cleaner dataset to publish
...
#### -> these matrices will start on the day of pairing and go to the day that the egg is terminated (the hatch.fail date)

temp.mn <- matrix(NA,nrow=nrow(enc.hist),ncol=ncol(enc.hist)) 
...
for(i in 1:nind){  
  for(t in first[i]:(last[i]-1)){
    ...
    logit(S[i,t]) <- beta.int + alpha.Pr[pair.ID[i]] + beta.fac[facility[i]]  #+ beta.temp.mn*temp.mn[i,t]
